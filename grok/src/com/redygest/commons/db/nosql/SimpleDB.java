...
import com.redygest.commons.util.Base64;
...
public class SimpleDB extends Object {
	 ...
	 * MaxNumberOfDomains domains. Calling ListDomains successive times with the
	...
	public List listDomains(int MaxNumberOfDomains, String NextToken)
			throws Exception {
		TreeMap uriParams = createStandardParams("ListDomains");

		if (MaxNumberOfDomains > 0)
			uriParams.put("MaxNumberOfDomains",
	 ...
	 */
	public String batchPutAttributes(String domainName,
			Map<String, Map<String, String>> itemValues) throws Exception {
		return batchPutAttributes(domainName, itemValues, new HashMap());
	}

	public String batchPutAttributes(String domainName,
			Map<String, Map<String, String>> itemValues,
			Map<String, Set> itemReplaces) throws Exception {
		TreeMap uriParams = createStandardParams("BatchPutAttributes");
		uriParams.put("DomainName", domainName);

		int itemCount = 0;
		for (Map.Entry<String, Map<String, String>> itemMap : itemValues
				.entrySet()) {
			...
			for (Map.Entry<String, String> x : map.entrySet()) {
				uriParams.put("Item." + itemCount + ".Attribute." + count
						+ ".Name", x.getKey());
				uriParams.put("Item." + itemCount + ".Attribute." + count
						+ ".Value", x.getValue());
		...
		uriParams
				.put("Signature", getSignature(false, httpEndPoint, uriParams));
		Map<String, String> results = postToConnection(new URL("https://"
				+ httpEndPoint), uriParams);
	 ...
	 * You specify new attributes using a combination of the Attribute.X.Name
	 * and Attribute.X.Value parameters. You specify the first attribute by the
	 * parameters Attribute.0.Name and Attribute.0.Value, the second attribute
	 * by the parameters Attribute.1.Name and Attribute.1.Value, and so on.
	 */
	public String putAttributes(String domainName, String itemName,
			Map<String, String> map) throws Exception {
		return putAttributes(domainName, itemName, map, null);
	}

	public String putAttributes(String domainName, String itemName,
			Map<String, String> map, Set replace) throws Exception {
	 ...
	 * 
	 * If the value of the map is null, then all the attributes of that name
	 * will be deleted
	 */
	public String deleteAttributes(String domainName, String itemName)
			throws Exception {
	...
	public String deleteAttributes(String domainName, String itemName,
		...
		if (map != null) {
			...
			for (Map.Entry<String, String> x : map.entrySet()) {
				...
				if (x.getValue() != null)
					uriParams
							.put("Attribute." + count + ".Value", x.getValue());
	 ...
	 */
	public Map getAttributes(String domainName, String itemName)
			throws Exception {
	...
	public Map getAttributes(String domainName, String itemName,
		...
		for (int x = 0; x < attributes.size(); x++) {
			...
			String key = t.substring(t.indexOf("<Name>") + 6,
					t.indexOf("</Name>"));
			String val = t.substring(t.indexOf("<Value>") + 7,
					t.indexOf("</Value>"));
	 ...
	 * enforce this limit. For example, even if you ask to retrieve 2500 items,
	 * but each individual item is 10 kB in size, the system returns 100 items
	...
	public List select(String selectExpression, String nextToken)
			throws Exception {
	...
	private String getSignature(boolean bGet, String http, TreeMap uriParams)
			throws Exception {
		...
		while (it.hasNext()) {
			...
			sb.append(key);
			sb.append("=");
			sb.append(URLEncoder.encode(val, "utf-8").replace("+", "%20")
	...
	public Map<String, String> postToConnection(URL endPoint, Map uriParams)
			throws Exception {
		...
		try {
			...
			while (it.hasNext()) {
				...
				writer.write(key);
				writer.write("=");
				writer.write(URLEncoder.encode(val, "utf-8")
