...
public class ConsoleFormatter
{
    ...
    public static void displayClusters(final Collection<Cluster> clusters, String file)
    {
        displayClusters(clusters, Integer.MAX_VALUE, file);
    }

    public static void displayClusters(final Collection<Cluster> clusters,
        int maxNumberOfDocumentsToShow, String file)
    {
        System.out.println("\n\nCreated " + clusters.size() + " clusters\n");
        int clusterNumber = 1;
        for (final Cluster cluster : clusters)
        {
            displayCluster(0, "" + clusterNumber++, cluster, maxNumberOfDocumentsToShow, file);
        }
    }
    ...
    private static String displayDocuments(final int level, Document document)
    {
    	String output ="";
        final String indent = getIndent(level);
        ...
        System.out.println(document.getField(Document.SUMMARY));
        output+=(document.getField(Document.SUMMARY)+"\n");
        final String url = document.getField(Document.CONTENT_URL);
        if (StringUtils.isNotBlank(url))
        {
            System.out.println(indent + "     " + url);
    ...
    {
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(file+ ".clusters.txt", true));
			...
			bw.write(label + " (" + cluster.getAllDocuments().size()
			...
			for (final Document document : cluster.getDocuments()) {
				...
				bw.write(displayDocuments(level + 1, document)+"\n");
				bw.flush();
				documentsShown++;
			}
			if (maxNumberOfDocumentsToShow > 0
					&& (cluster.getDocuments().size() > documentsShown)) {
				bw.write(getIndent(level + 1) + "... and "
						+ (cluster.getDocuments().size() - documentsShown)
						+ " more\n");
			...
			for (final Cluster subcluster : cluster.getSubclusters()) {
				displayCluster(level + 1, tag + "." + num, subcluster,
						maxNumberOfDocumentsToShow, file);
			}
			bw.flush();
		} catch (Exception e) {
			e.printStackTrace();
